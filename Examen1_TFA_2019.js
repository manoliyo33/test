// ============================================================
// ==================== EXAMEN 1 ===============================
// ============================================================
// Técnico/a de Función Administrativa, Sistemas y Tecnología de la Información
// Acceso Libre 2019

const examen1_TFA_2019 = [
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cómo se clasifica el personal estatutario de gestión y servicios, según el artículo 7 del Estatuto Marco (Ley 55/2003, de 16 de diciembre)?",
        options: [
            "Personal de formación universitaria y personal de oficio",
            "Otro personal, personal de formación profesional y personal de formación universitaria",
            "Personal licenciado universitario, técnicos no sanitarios y personal de otras categorías",
            "Personal diplomado o graduado universitario, personal técnico y personal de oficio"
        ],
        correctAnswer: 1,
        feedback: "El artículo 7 del Estatuto Marco clasifica al personal estatutario de gestión y servicios en otro personal, personal de formación profesional y personal de formación universitaria."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuáles son los principios y criterios que el Estatuto Marco (Ley 55/2003), en su artículo 4.b), establece en el acceso a la condición de personal estatutario?",
        options: [
            "Igualdad, mérito, capacidad y publicidad",
            "Mérito, carrera profesional y transparencia",
            "Igualdad de género, acreditación profesional, titulación y transparencia",
            "Justicia, igualdad y prueba selectiva"
        ],
        correctAnswer: 0,
        feedback: "El artículo 4.b) del Estatuto Marco establece como principios de acceso: igualdad, mérito, capacidad y publicidad."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿A qué vacaciones anuales retribuidas tiene derecho el personal estatutario, según recoge el artículo 53 del Estatuto Marco?",
        options: [
            "A 23 días laborables, o al tiempo que proporcionalmente corresponda en función del tiempo de servicios",
            "A 25 días laborables, o al tiempo proporcional al período trabajado",
            "A una cuya duración no será inferior a 30 días naturales, o al tiempo que proporcionalmente corresponda en función del tiempo de servicios",
            "A una de duración no inferior a 22 días naturales, o al tiempo que proporcionalmente corresponda en función del tiempo de servicios"
        ],
        correctAnswer: 2,
        feedback: "El artículo 53 del Estatuto Marco establece que la duración de las vacaciones no será inferior a 30 días naturales."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el artículo 9.2 de la Ley 41/2002, ¿en qué casos los facultativos podrán llevar a cabo intervenciones clínicas sin necesidad de consentimiento?",
        options: [
            "Cuando el paciente no sea capaz de tomar decisiones, a criterio del médico responsable",
            "Cuando el paciente tenga la capacidad modificada judicialmente y así conste en la sentencia",
            "Cuando el paciente menor de edad no sea capaz intelectual ni emocionalmente de comprender el alcance de la intervención",
            "Cuando existe riesgo inmediato grave para la integridad física o psíquica del enfermo y no es posible conseguir su autorización"
        ],
        correctAnswer: 3,
        feedback: "El artículo 9.2 de la Ley 41/2002 establece la excepción por riesgo inmediato grave para la integridad física o psíquica del enfermo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Los centros sanitarios tienen la obligación de conservar la documentación clínica. ¿Cuál es el período mínimo de conservación establecido en la Ley 41/2002?",
        options: [
            "Cinco años, contados desde la fecha del alta de cada proceso asistencial",
            "Cuatro años, contados desde la fecha del alta de cada proceso asistencial",
            "Veinte años, contados desde la fecha del alta del último proceso asistencial",
            "Tres años, contados desde la fecha del alta del último proceso asistencial"
        ],
        correctAnswer: 0,
        feedback: "La Ley 41/2002 establece un período mínimo de conservación de cinco años desde la fecha del alta de cada proceso asistencial."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación con el derecho de los pacientes a la información asistencial (Ley 41/2002), indique cuál de las siguientes afirmaciones NO es correcta:",
        options: [
            "La información siempre se proporcionará por escrito, dejando constancia en la historia clínica",
            "Toda persona tiene derecho a que se respete su voluntad de no ser informada",
            "El derecho a la información sanitaria puede limitarse por la existencia acreditada de un estado de necesidad terapéutica",
            "El paciente será informado incluso en caso de incapacidad, de modo adecuado a sus posibilidades de comprensión"
        ],
        correctAnswer: 0,
        feedback: "La información no siempre debe proporcionarse por escrito; puede ser verbal, dejando constancia en la historia clínica."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "El artículo 10.1 de la Ley 41/2002 establece la información básica que el facultativo proporcionará al paciente. Señale cuál corresponde exactamente a uno de los contenidos de esta información básica:",
        options: [
            "Las consecuencias que la intervención pueda originar",
            "Las contraindicaciones",
            "Solo contendrá los riesgos graves relacionados con las circunstancias personales o profesionales del paciente",
            "Los riesgos improbables en condiciones excepcionales"
        ],
        correctAnswer: 0,
        feedback: "El artículo 10.1 de la Ley 41/2002 establece que la información básica debe incluir las consecuencias que la intervención pueda originar."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Dentro de la organización territorial de los servicios sanitarios que establece la Ley de Salud de Andalucía (Ley 2/1998), cada área de salud se divide territorialmente en:",
        options: ["Zonas Básicas de Salud", "Distritos Sanitarios", "Centros de Salud", "Áreas de Gestión Sanitaria"],
        correctAnswer: 0,
        feedback: "La Ley 2/1998 establece que cada área de salud se divide territorialmente en Zonas Básicas de Salud."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el Estatuto de Autonomía de Andalucía, el Defensor del Pueblo Andaluz:",
        options: [
            "Será elegido por el Parlamento por mayoría cualificada",
            "Será elegido por el Consejo de Gobierno por mayoría cualificada",
            "Será elegido por el Parlamento por mayoría simple",
            "Será elegido por el Presidente del Parlamento"
        ],
        correctAnswer: 0,
        feedback: "El Defensor del Pueblo Andaluz es elegido por el Parlamento por mayoría cualificada."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la Ley de Prevención de Riesgos Laborales, el órgano paritario y colegiado de participación para la consulta regular en materia de prevención se llama:",
        options: ["Unidad de Prevención de Riesgos Laborales", "Comité de Vigilancia de la Salud", "Comité de Seguridad y Salud", "Comité de Delegados de Prevención"],
        correctAnswer: 2,
        feedback: "El Comité de Seguridad y Salud es el órgano paritario y colegiado de participación en materia de prevención de riesgos laborales."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la Ley General de Sanidad (artículo 15), para atender a pacientes que superan las posibilidades de diagnóstico y tratamiento de su Comunidad Autónoma, se prevé:",
        options: [
            "El acceso a servicios de referencia, acreditados por el Ministerio de Sanidad",
            "La derivación a centros privados concertados con el SNS",
            "La derivación a centros sanitarios privados ubicados en su propia Comunidad Autónoma",
            "La derivación a centros sanitarios de referencia extranjeros"
        ],
        correctAnswer: 0,
        feedback: "El artículo 15 de la Ley General de Sanidad establece el acceso a servicios de referencia acreditados por el Ministerio de Sanidad."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Qué artículo de la Constitución Española fue modificado (15/02/2024) para cambiar el término 'disminuidos' por 'personas con discapacidad'?",
        options: ["Artículo 25", "Artículo 29", "Artículo 31", "Artículo 49"],
        correctAnswer: 3,
        feedback: "El artículo 49 de la Constitución Española fue modificado para cambiar el término por 'personas con discapacidad'."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el artículo 43 de la Constitución Española, son ciertas las siguientes respuestas EXCEPTO:",
        options: [
            "Se reconoce el derecho a la protección de la salud",
            "Compete a los poderes públicos organizar y tutelar la salud pública",
            "Los poderes públicos fomentarán la educación sanitaria, la educación física y el deporte",
            "Los poderes públicos fomentarán la educación sanitaria, la educación física y la alimentación equilibrada"
        ],
        correctAnswer: 3,
        feedback: "El artículo 43 no menciona la 'alimentación equilibrada'; se refiere a educación sanitaria, educación física y deporte."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la Ley Orgánica 3/2018, de Protección de Datos, ¿cuál de los siguientes supuestos NO está excluido de su ámbito de aplicación?",
        options: [
            "Los tratamientos excluidos del ámbito de aplicación del RGPD",
            "Los tratamientos de datos de personas fallecidas",
            "Los tratamientos sometidos a la normativa sobre protección de materias clasificadas",
            "Los tratamientos llevados a cabo por órganos judiciales"
        ],
        correctAnswer: 1,
        feedback: "La Ley Orgánica 3/2018 sí se aplica a los tratamientos de datos de personas fallecidas, según lo establecido en su artículo 3."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la Ley 9/2017, de Contratos del Sector Público, ¿cuál de los siguientes contratos tiene la consideración de contrato menor?",
        options: [
            "Un contrato de servicios con un valor estimado de 35.000 euros",
            "Un contrato de suministros con un valor estimado de 17.500 euros",
            "Un contrato de obras con un valor estimado de 40.000 euros",
            "Un contrato de concesión de obras con un valor estimado de 40.000 euros"
        ],
        correctAnswer: 0,
        feedback: "Un contrato de servicios de 35.000 euros es un contrato menor según la Ley 9/2017 (límite 50.000€ para servicios)."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "De acuerdo con la Ley 9/2017, ¿cuál de los siguientes principios debe regir la contratación del sector público?",
        options: [
            "Competencia desleal, publicidad limitada y discrecionalidad",
            "Confidencialidad, libre designación y beneficio económico",
            "Libertad de elección, contratación directa y urgencia",
            "Libertad de acceso a las licitaciones, publicidad y transparencia de los procedimientos"
        ],
        correctAnswer: 3,
        feedback: "La Ley 9/2017 establece como principios: libertad de acceso, publicidad, transparencia, no discriminación e igualdad de trato."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el artículo 1.1 de la Ley 9/2017, ¿qué se entiende por contrato oneroso?",
        options: [
            "Cuando el contratista obtenga algún tipo de beneficio económico, ya sea de forma directa o indirecta",
            "Cuando el contratista obtenga algún tipo de beneficio económico de forma directa",
            "Cuando la Administración Pública obtenga algún tipo de beneficio económico",
            "Ninguna de las anteriores"
        ],
        correctAnswer: 0,
        feedback: "El contrato oneroso es aquel en el que el contratista obtiene algún tipo de beneficio económico, directo o indirecto."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el artículo 14.2 de la Ley 39/2015, ¿qué sujetos están obligados a relacionarse electrónicamente con las Administraciones Públicas?",
        options: [
            "Cualquier ciudadano mayor de edad",
            "Las personas físicas que presenten escritos en nombre propio",
            "Los empleados de las Administraciones Públicas para los trámites y actuaciones que realicen con ellas por razón de su condición de empleado público",
            "Sólo las empresas que contraten con la Administración"
        ],
        correctAnswer: 2,
        feedback: "El artículo 14.2 de la Ley 39/2015 establece que los empleados públicos están obligados a relacionarse electrónicamente."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Qué técnica se utiliza en la gestión por procesos para mapear visualmente un flujo de trabajo?",
        options: ["Análisis PESTEL", "Diagrama de Ishikawa", "Mapeo de procesos (Business Process Mapping)", "Benchmarking"],
        correctAnswer: 2,
        feedback: "El mapeo de procesos se utiliza para representar visualmente un flujo de trabajo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación con la Estrategia de Salud Digital de Andalucía, ¿cuál de los siguientes objetivos se encuentra más alineado con la evolución de los sistemas de información asistenciales del SAS?",
        options: [
            "Unificación de sistemas departamentales locales de cada hospital",
            "Garantizar la trazabilidad del dato clínico mediante arquitectura de historia clínica distribuida",
            "Fortalecer los sistemas de información corporativos con enfoque en la satisfacción del personal facultativo",
            "Minimizar la visibilidad de datos clínicos para evitar problemas de confidencialidad y privacidad"
        ],
        correctAnswer: 1,
        feedback: "La Estrategia de Salud Digital de Andalucía busca garantizar la trazabilidad del dato clínico mediante historia clínica distribuida."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el contexto del SAS, ¿qué es 'Ventana Abierta a la Familia' y cuál es su propósito principal?",
        options: [
            "Un programa de formación para profesionales sanitarios sobre atención familiar",
            "Una plataforma que permite a las familias acceder a información y servicios relacionados con la salud infantil",
            "Un servicio de atención psicológica dirigido a familias con miembros en tratamiento médico prolongado",
            "Un sistema para consultas médicas familiares a distancia"
        ],
        correctAnswer: 1,
        feedback: "Ventana Abierta a la Familia es una plataforma para que las familias accedan a información sobre salud infantil."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el uso de hojas de cálculo avanzadas, ¿qué funcionalidad permite realizar análisis dinámico de grandes volúmenes de datos sin modificar los datos originales?",
        options: [
            "Formularios de entrada vinculados a macros",
            "Tablas dinámicas (o Pivot Tables)",
            "Referencias circulares para iteración automática",
            "Validación de datos mediante expresiones regulares"
        ],
        correctAnswer: 1,
        feedback: "Las tablas dinámicas permiten analizar grandes volúmenes de datos sin modificar los datos originales."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En un sistema de gestión documental, ¿qué estándar debe tenerse en cuenta para asegurar la interoperabilidad con el archivo electrónico administrativo en España?",
        options: ["EI ENS", "ISO/IEC 27001", "NTI de Política de Gestión de Documentos Electrónicos del ENI", "Ley 9/2017"],
        correctAnswer: 2,
        feedback: "La NTI de Política de Gestión de Documentos Electrónicos del ENI asegura la interoperabilidad con el archivo electrónico administrativo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Qué técnica permite mejorar la capacidad y flexibilidad de almacenamiento en un centro de datos sin necesidad de añadir más servidores físicos?",
        options: [
            "Implementación de almacenamiento en la nube con capacidad bajo demanda",
            "Configuración de RAID 0 para aumentar la velocidad de escritura sin redundancia",
            "Migración de todos los datos a unidades de estado sólido (SSD)",
            "Reducción de la resolución de archivos multimedia almacenados en el servidor"
        ],
        correctAnswer: 0,
        feedback: "El almacenamiento en la nube con capacidad bajo demanda permite mejorar la capacidad y flexibilidad sin añadir servidores físicos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En un entorno donde se requiere alta disponibilidad, ¿qué factor es crítico para evitar interrupciones en el servicio?",
        options: [
            "Garantizar redundancia en los componentes críticos del sistema",
            "Utilizar servidores con la mayor cantidad posible de memoria RAM",
            "Evitar el uso de redes de respaldo para reducir costos de infraestructura",
            "Programar mantenimientos frecuentes que impliquen apagados completos del sistema"
        ],
        correctAnswer: 0,
        feedback: "La redundancia en componentes críticos es fundamental para garantizar la alta disponibilidad."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de las siguientes afirmaciones sobre SNOMED CT es correcta?",
        options: [
            "Es un sistema de codificación exclusivamente diseñado para la facturación de procedimientos médicos",
            "Organiza los conceptos clínicos en una estructura jerárquica con relaciones definidas",
            "Sólo se usa en Estados Unidos, no cuenta con reconocimiento a nivel internacional",
            "No permite la interoperabilidad con otros estándares como HL7 FHIR"
        ],
        correctAnswer: 1,
        feedback: "SNOMED CT organiza los conceptos clínicos en una estructura jerárquica con relaciones definidas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la herramienta PILAR, utilizada para el análisis y gestión de riesgos, ¿qué función principal desempeña el 'perfil de seguridad'?",
        options: [
            "Cuantifica el impacto financiero y reputacional de cada vulnerabilidad identificada",
            "Asigna responsables de seguridad dentro de la organización",
            "Genera un inventario detallado de todos los activos físicos de la infraestructura",
            "Define los umbrales de seguridad que deben cumplir los sistemas en función de su categorización"
        ],
        correctAnswer: 3,
        feedback: "El perfil de seguridad en PILAR define los umbrales de seguridad que deben cumplir los sistemas según su categorización."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En España, dentro de la Plataforma CL@VE, ¿cuál de los siguientes mecanismos proporciona el mayor nivel de seguridad en autenticación electrónica?",
        options: [
            "Cl@ve PIN, basado en un código de un sólo uso enviado por SMS",
            "Cl@ve Permanente, utilizando usuario y contraseña con verificación OTP",
            "DNIe y Certificado Electrónico, basado en criptografía de clave pública",
            "Acceso con credenciales delegadas verificadas"
        ],
        correctAnswer: 2,
        feedback: "El DNIe y Certificado Electrónico proporcionan el mayor nivel de seguridad al usar criptografía de clave pública."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el contexto de los protocolos de transferencia de ficheros, ¿qué ventaja aporta SFTP frente a FTP y FTPS?",
        options: [
            "SFTP requiere menos recursos computacionales que FTP al utilizar cifrado parcial",
            "SFTP cifra tanto el canal de control como el de datos mediante una única conexión segura",
            "SFTP permite la autenticación mediante certificados X.509, a diferencia de FTPS",
            "SFTP usa el mismo puerto y estructura de comandos que FTP"
        ],
        correctAnswer: 1,
        feedback: "SFTP cifra tanto el canal de control como el de datos mediante una única conexión segura."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál es una diferencia clave entre un sistema de gestión documental tradicional y un espacio colaborativo tipo Confluence en cuanto al control de versiones?",
        options: [
            "Confluence almacena documentos sin posibilidad de control de versiones",
            "Los gestores documentales tradicionales ofrecen versiones automáticas sólo en archivos PDF",
            "Confluence organiza el contenido en páginas jerárquicas con versionado integrado, mientras los gestores documentales lo hacen sobre archivos independientes",
            "Los gestores documentales no permiten colaboración simultánea en entornos web"
        ],
        correctAnswer: 2,
        feedback: "Confluence organiza el contenido en páginas jerárquicas con versionado integrado, mientras los gestores documentales trabajan sobre archivos independientes."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el estándar DICOM, ¿qué protocolo de red se emplea para la transmisión de imágenes médicas?",
        options: [
            "SOAP sobre HTTP",
            "TLS sobre TCP/IP con codificación Base64",
            "C-STORE sobre el protocolo DIMSE basado en TCP/IP",
            "MQTT con cifrado AES-256"
        ],
        correctAnswer: 2,
        feedback: "DICOM utiliza C-STORE sobre el protocolo DIMSE basado en TCP/IP para la transmisión de imágenes médicas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la metodología LEAN aplicada al desarrollo de software, ¿cuál de los siguientes conceptos describe mejor el objetivo de eliminar desperdicios ('waste')?",
        options: [
            "Minimizar la documentación del código fuente para reducir tiempos de desarrollo",
            "Evitar el desarrollo de características que no aporten valor real al usuario final",
            "Reducir la interacción con el cliente para evitar retrasos en la toma de decisiones",
            "Limitar la refactorización del código para reducir el número de iteraciones en el desarrollo"
        ],
        correctAnswer: 1,
        feedback: "En LEAN, eliminar desperdicios significa evitar el desarrollo de características que no aporten valor real al usuario final."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según MAGERIT, ¿cuál es la diferencia clave entre un análisis de impacto y un análisis de riesgos?",
        options: [
            "El análisis de impacto se centra en las consecuencias de una amenaza, mientras que el análisis de riesgos considera tanto la probabilidad como el impacto",
            "El análisis de impacto sólo evalúa daños económicos, mientras que el análisis de riesgos considera factores técnicos y organizativos",
            "El análisis de riesgos se aplica únicamente a sistemas informáticos, mientras que el análisis de impacto se enfoca en la continuidad del negocio",
            "El análisis de impacto es opcional en MAGERIT, mientras que el análisis de riesgos es obligatorio"
        ],
        correctAnswer: 0,
        feedback: "El análisis de impacto se centra en las consecuencias de una amenaza, mientras que el análisis de riesgos considera tanto probabilidad como impacto."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál es la principal diferencia entre técnicas descriptivas y predictivas en Inteligencia Artificial?",
        options: [
            "Las técnicas descriptivas generan modelos supervisados, mientras que las predictivas sólo utilizan datos sin etiquetar",
            "Las técnicas predictivas identifican patrones sin necesidad de variables objetivo",
            "Las técnicas descriptivas buscan entender estructuras y relaciones en los datos, mientras que las predictivas hacen estimaciones sobre datos futuros",
            "Las técnicas descriptivas sólo se aplican en bases de datos estructuradas"
        ],
        correctAnswer: 2,
        feedback: "Las técnicas descriptivas buscan entender estructuras y relaciones en los datos, mientras que las predictivas hacen estimaciones sobre datos futuros."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el Esquema Nacional de Seguridad (ENS), ¿cuál es la principal diferencia entre las medidas de seguridad de nivel ALTO y nivel MEDIO?",
        options: [
            "Las medidas de nivel ALTO requieren la implementación obligatoria de un SOC",
            "En el nivel ALTO, se exige una auditoría de seguridad con periodicidad máxima de 1 año, mientras que en el nivel MEDIO es cada 2 años",
            "Las organizaciones con nivel MEDIO no están obligadas a implementar planes de continuidad del servicio",
            "El nivel MEDIO permite el uso de contraseñas sin autenticación multifactor"
        ],
        correctAnswer: 1,
        feedback: "En el nivel ALTO, se exige una auditoría de seguridad con periodicidad máxima de 1 año, mientras que en el nivel MEDIO es cada 2 años."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el marco del Reglamento eIDAS2, ¿cuál de los siguientes elementos es obligatorio para los monederos de identidad digital (EUID Wallets)?",
        options: [
            "Almacenamiento de datos biométricos en una base de datos centralizada",
            "Posibilidad de que los ciudadanos gestionen qué atributos de su identidad comparten con terceros",
            "Uso exclusivo para autenticación en servicios gubernamentales",
            "Integración con sistemas de firma electrónica exclusivamente en formato PAdES"
        ],
        correctAnswer: 1,
        feedback: "eIDAS2 exige que los ciudadanos puedan gestionar qué atributos de su identidad comparten con terceros."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la jerarquía de memoria de un sistema informático, ¿qué efecto puede tener la inclusión de una caché de nivel L3 en el rendimiento de la CPU?",
        options: [
            "Disminuye el tiempo medio de acceso a la memoria en todas las cargas de trabajo",
            "Sólo mejora el rendimiento en aplicaciones que hacen uso intensivo de cálculos en coma flotante",
            "Actúa como un buffer de alta capacidad para reducir la tasa de fallos en la memoria RAM",
            "No tiene impacto significativo en el rendimiento"
        ],
        correctAnswer: 0,
        feedback: "La caché L3 disminuye el tiempo medio de acceso a la memoria en la mayoría de las cargas de trabajo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En un entorno cliente-servidor, una de las características principales es:",
        options: [
            "No diferenciar capas ni funciones dentro de la arquitectura",
            "Ejecutar todos los procesos y la lógica de negocio en el cliente",
            "No existir comunicación en red: todo ocurre en un único equipo físico",
            "El cliente solicita servicios y el servidor los provee, separando la capa de presentación de la lógica y datos"
        ],
        correctAnswer: 3,
        feedback: "En cliente-servidor, el cliente solicita servicios y el servidor los provee, separando la capa de presentación de la lógica y datos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "La iniciativa de 'supresión de certificados en soporte papel' en la administración pública busca:",
        options: [
            "Obligar a los ciudadanos a presentar más documentos en ventanilla",
            "Evitar la necesidad de aportar copias en papel de ciertos certificados, facilitando que las administraciones consulten o intercambien los datos electrónicamente",
            "Imponer el uso de fax en todos los trámites oficiales",
            "Eliminar cualquier tipo de registro administrativo digital"
        ],
        correctAnswer: 1,
        feedback: "La supresión de certificados en papel busca evitar la necesidad de aportar copias en papel, facilitando el intercambio electrónico de datos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "La clave primaria en una tabla relacional se caracteriza por:",
        options: [
            "Ser un atributo o grupo de atributos que identifica de forma única cada fila y no admite valores repetidos ni nulos",
            "Ser siempre la primera columna definida en la tabla",
            "Permitir valores duplicados si los demás campos coinciden",
            "Ser un atributo opcional que puede repetirse sin inconvenientes"
        ],
        correctAnswer: 0,
        feedback: "La clave primaria identifica de forma única cada fila y no admite valores repetidos ni nulos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Qué es el desarrollo low-code?",
        options: [
            "Un método para desarrollar software exclusivamente en sistemas mainframe",
            "Un procedimiento que se basa en reescribir cada línea de código a bajo nivel",
            "Un modelo de desarrollo de software que emplea principalmente lenguajes de máquina",
            "Un modelo de desarrollo de software que permite crear aplicaciones con mínimas líneas de código y con herramientas gráficas"
        ],
        correctAnswer: 3,
        feedback: "El desarrollo low-code permite crear aplicaciones con mínimas líneas de código y con herramientas gráficas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "El protocolo HTTPS consiste en:",
        options: [
            "Un método para compartir archivos en redes locales sin autenticación",
            "El uso de HTTP sin ningún tipo de cifrado",
            "Un protocolo P2P diseñado para transmisión de vídeo en tiempo real",
            "Ejecutar HTTP sobre una conexión cifrada con SSL/TLS"
        ],
        correctAnswer: 3,
        feedback: "HTTPS es HTTP ejecutado sobre una conexión cifrada con SSL/TLS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Al diseñar un sistema de información con altos requisitos de seguridad, ¿cuál de las siguientes buenas prácticas suele recomendarse?",
        options: [
            "No emplear cifrado para agilizar el tráfico de datos",
            "Implementar mecanismos de autenticación y autorización, además de cifrar los datos",
            "Permitir conexiones anónimas sin restricciones para facilitar la puesta en marcha",
            "Todas las respuestas anteriores son correctas"
        ],
        correctAnswer: 1,
        feedback: "Se recomienda implementar mecanismos de autenticación y autorización, además de cifrar los datos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la construcción de sistemas de información, el uso de repositorios Git facilita:",
        options: [
            "Eliminar la necesidad de pruebas unitarias y de integración",
            "Que sólo un desarrollador trabaje en el proyecto mientras el resto espera",
            "Ignorar por completo la gestión de la configuración",
            "El seguimiento detallado de cambios, la colaboración simultánea entre varios desarrolladores y la posibilidad de revertir o fusionar versiones"
        ],
        correctAnswer: 3,
        feedback: "Git facilita el seguimiento detallado de cambios, la colaboración simultánea entre varios desarrolladores y la posibilidad de revertir o fusionar versiones."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "La usabilidad en el diseño web se relaciona directamente con:",
        options: [
            "La facilidad de aprendizaje, eficacia, eficiencia y satisfacción que experimentan los usuarios al interactuar con una interfaz",
            "La estética puramente gráfica sin tener en cuenta la experiencia del usuario",
            "La capacidad del sitio para bloquear accesos no autorizados",
            "El volumen de datos transmitidos por segundo"
        ],
        correctAnswer: 0,
        feedback: "La usabilidad se relaciona con la facilidad de aprendizaje, eficacia, eficiencia y satisfacción del usuario."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Una de las características más destacadas al adoptar software libre en la administración pública es:",
        options: [
            "No poder acceder nunca al código fuente ni auditar su funcionamiento interno",
            "La dependencia total de un único proveedor que controle la evolución del software",
            "La reducción de costes de licenciamiento y la independencia de proveedores, permitiendo adaptar el software a necesidades específicas",
            "La imposibilidad de realizar desarrollos propios o mejoras a medida"
        ],
        correctAnswer: 2,
        feedback: "El software libre permite reducir costes de licenciamiento y la independencia de proveedores, adaptando el software a necesidades específicas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Un inconveniente que con frecuencia se menciona sobre el software libre y de código abierto es:",
        options: [
            "La posible falta de soporte oficial o inmediato",
            "La ausencia absoluta de documentación en cada proyecto",
            "La licencia siempre es privativa, impidiendo su redistribución",
            "La imposibilidad de modificar libremente el código fuente en caso de bugs"
        ],
        correctAnswer: 0,
        feedback: "Un inconveniente del software libre es la posible falta de soporte oficial o inmediato."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Un bus de interoperabilidad o ESB:",
        options: [
            "Es un canal físico para transferir datos mediante discos externos",
            "Orquesta y media la comunicación entre servicios, facilitando la transformación de mensajes, enrutamiento y monitorización",
            "Limita la escalabilidad del sistema al obligar a usar un único protocolo de comunicación",
            "Sólo aplica en arquitecturas de dos niveles"
        ],
        correctAnswer: 1,
        feedback: "Un ESB orquesta y media la comunicación entre servicios, facilitando la transformación de mensajes, enrutamiento y monitorización."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "La normalización en el diseño de bases de datos persigue principalmente:",
        options: [
            "Organizar toda la información en una única tabla para facilitar su consulta",
            "Repetir los mismos datos en múltiples ubicaciones",
            "Establecer como norma que todos los datos deben almacenarse únicamente en texto",
            "Estructurar los datos en distintas tablas normalizadas para minimizar la redundancia y evitar inconsistencias en su almacenamiento"
        ],
        correctAnswer: 3,
        feedback: "La normalización busca estructurar los datos en distintas tablas para minimizar la redundancia y evitar inconsistencias."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la elaboración de prototipos, una de las ventajas principales es:",
        options: [
            "La generación de un producto final sin requerir pruebas posteriores",
            "La ausencia de retroalimentación por parte de los usuarios",
            "La rápida obtención de versiones preliminares que ayudan a concretar requisitos",
            "La eliminación total de los riesgos del proyecto"
        ],
        correctAnswer: 2,
        feedback: "La elaboración de prototipos permite obtener rápidamente versiones preliminares que ayudan a concretar requisitos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En la generación automática de código, una de las ventajas principales es:",
        options: [
            "La imposibilidad de adaptar la lógica de negocio a necesidades específicas",
            "Obligar a que todo el código sea monolítico sin posibilidad de modularidad",
            "Aumentar la complejidad del sistema y la curva de aprendizaje del equipo",
            "Disminuir los errores humanos al automatizar la creación de partes repetitivas o plantillas estándar del código"
        ],
        correctAnswer: 3,
        feedback: "La generación automática de código disminuye los errores humanos al automatizar la creación de partes repetitivas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En Python, para la siguiente definición de bucle 'for i in [10]': ¿cuántas veces se ejecutará el bucle?",
        options: ["10", "2", "1", "0"],
        correctAnswer: 2,
        feedback: "El bucle se ejecutará 1 vez, ya que itera sobre una lista con un solo elemento (10)."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de las siguientes características distingue principalmente a HTTP/2 de HTTP/1.1?",
        options: [
            "El uso de cabeceras en texto plano para cada petición y respuesta",
            "El requerimiento de utilizar siempre un protocolo UDP en vez de TCP",
            "La multiplexación de varias peticiones y respuestas en una única conexión",
            "El abandono completo de TLS/SSL como método de cifrado"
        ],
        correctAnswer: 2,
        feedback: "HTTP/2 introduce la multiplexación de varias peticiones y respuestas en una única conexión."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el ámbito del modelado de dominio, un modelo Entidad–Relación se utiliza para:",
        options: [
            "Definir la estructura de datos de forma conceptual",
            "Representar gráficamente los procesos de negocio y sus flujos de actividad",
            "Describir la interacción temporal entre objetos del sistema",
            "Identificar los requerimientos no funcionales del sistema"
        ],
        correctAnswer: 0,
        feedback: "El modelo Entidad-Relación se utiliza para definir la estructura de datos de forma conceptual."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Una característica de los ficheros indexados es que:",
        options: [
            "Se utilizan para compartir información en redes P2P",
            "Almacenan los datos de forma secuencial e incorporan un índice auxiliar que permite localizar registros rápidamente",
            "Su uso principal se orienta al almacenamiento estructurado de datos exclusivamente en formatos como JSON",
            "No permiten realizar operaciones de inserción o eliminación de registros"
        ],
        correctAnswer: 1,
        feedback: "Los ficheros indexados almacenan los datos de forma secuencial e incorporan un índice auxiliar para localizar registros rápidamente."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el catálogo de componentes del SAS, ¿cuál es el criterio principal que define la estructura del nombre de estos componentes?",
        options: [
            "Se compone de un espacio de nombres (@sas/) y un prefijo (wc-stic-), seguido de la descripción del componente",
            "Se compone de un espacio de nombres (@sas/) y un prefijo (wc-stic-), en el que la parte final está compuesta únicamente por la numeración de la versión",
            "Se define de forma aleatoria por el equipo que desarrolla el componente",
            "Se asigna un identificador automático e incremental para cada nuevo componente"
        ],
        correctAnswer: 0,
        feedback: "La estructura del nombre se compone de un espacio de nombres (@sas/) y un prefijo (wc-stic-), seguido de la descripción del componente."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la Base Poblacional de Salud (BPS) del Sistema Sanitario Público de Andalucía, la información acerca del uso de recursos sanitarios se centra en:",
        options: [
            "Mostrar los datos administrativo-clínicos en base al CMBD hospitalario y la Historia Clínica Digital (Diraya)",
            "Mostrar las consultas de AP y AE, las urgencias, los distintos procesos de atención hospitalaria y el consumo de farmacia",
            "Mostrar la información demográfica recogida a través de su integración con BDU",
            "Ninguna de las respuestas es correcta"
        ],
        correctAnswer: 1,
        feedback: "La BPS muestra las consultas de AP y AE, urgencias, procesos de atención hospitalaria y consumo de farmacia."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de las siguientes metodologías se utilizan para abordar la gestión de cambios en las organizaciones?",
        options: ["Modelo de Kurt Lewin", "Modelo de Kotter", "ADKAR (Prosic) incluido en HCMBOK", "Todas las respuestas son correctas"],
        correctAnswer: 3,
        feedback: "Todos los modelos mencionados (Kurt Lewin, Kotter y ADKAR) se utilizan para abordar la gestión de cambios."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En ITIL v4 se definen una serie de prácticas. ¿Cuál de las siguientes es una práctica de gestión general?",
        options: ["Gestión del talento y personal", "Análisis de negocio", "Gestión de catálogo de servicios de TI", "Gestión de diseño y transición"],
        correctAnswer: 2,
        feedback: "La gestión de catálogo de servicios de TI es una práctica de gestión general en ITIL v4."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el Modelo Corporativo Marco de Implantaciones (MCMI) del SAS, ¿cuál de las siguientes actividades se realiza en la fase de 'Reingeniería de procesos'?",
        options: [
            "[ACT-RP-FUNC-5] Verificar estado inicial",
            "[ACT-RP-FUNC-1] Definir impacto de la implantación",
            "[ACT-RP-FUNC-8] Definir Plan de formación",
            "[ACT-RP-SIST-3] Catalogar los sistemas de información"
        ],
        correctAnswer: 1,
        feedback: "En la fase de Reingeniería de procesos se realiza la actividad [ACT-RP-FUNC-1] Definir impacto de la implantación."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de los siguientes es un sistema de información del ámbito de recursos humanos del SAS?",
        options: ["Código Numérico Profesional", "FARO, herramienta de gestión de recursos humanos", "Red de datos de profesionales", "Todas las respuestas son correctas"],
        correctAnswer: 3,
        feedback: "Todos los sistemas mencionados son sistemas de información del ámbito de recursos humanos del SAS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "El artículo 22 de la Ley 9/2017 indica el umbral de los contratos de servicios sujetos a regulación armonizada para entidades distintas a la AGE. ¿Cuál es ese umbral?",
        options: ["143.000 euros", "221.000 euros", "750.000 euros", "Ninguna de las respuestas es correcta"],
        correctAnswer: 0,
        feedback: "El umbral para contratos de servicios sujetos a regulación armonizada es de 143.000 euros."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "AviSAS es el sistema de información del SAS para notificación de eventos. ¿Cuál de los siguientes NO ES un canal actualmente disponible para comunicación con los ciudadanos?",
        options: ["Correo electrónico", "Correo postal", "Notificación push en App Salud Andalucía", "Mensaje SMS"],
        correctAnswer: 1,
        feedback: "El correo postal NO es un canal actualmente disponible en AviSAS para la comunicación con los ciudadanos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En un diagrama de PERT, ¿a qué tipo de holgura corresponde el tiempo suplementario para una actividad si las precedentes terminaran lo más tarde posible y las posteriores lo antes posible?",
        options: ["Holgura libre", "Holgura total", "Holgura independiente", "Holgura condicional"],
        correctAnswer: 2,
        feedback: "La holgura independiente es el tiempo suplementario para una actividad si las precedentes terminan lo más tarde posible y las posteriores lo antes posible."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el ámbito de la explotación de datos, ¿qué es un 'Data Fabric'?",
        options: [
            "Una arquitectura de servicios y funcionalidades que contribuye a procesar mejor los volúmenes de datos procedentes de multitud de fuentes con la capacidad de agruparlos bajo un sistema de administración",
            "Un repositorio de almacenamiento centralizado que contiene gran cantidad de datos en bruto",
            "Una base de datos corporativa para la integración y depuración de información procedente de múltiples fuentes",
            "Un subconjunto de los datos guardados en un Data Warehouse"
        ],
        correctAnswer: 0,
        feedback: "Data Fabric es una arquitectura de servicios que procesa volúmenes de datos de múltiples fuentes bajo un sistema de administración."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Tras realizar el análisis de la situación actual, la matriz CAME permite definir estrategias. ¿Cuál de las siguientes opciones corresponde a una de las estrategias de la matriz CAME?",
        options: [
            "Corregir las debilidades detectadas en el análisis",
            "Asegurar que se han identificado los puntos de mejora de la organización",
            "Minimizar la desviación de costes asociados a la ejecución de los proyectos estratégicos",
            "Elaborar un conjunto de lecciones aprendidas que favorezcan la mejora continua"
        ],
        correctAnswer: 0,
        feedback: "La matriz CAME incluye la estrategia de 'Corregir las debilidades detectadas en el análisis'."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el contexto del procesamiento analítico en línea (OLAP), ¿cuál de las siguientes opciones representa correctamente una clasificación de los sistemas OLAP?",
        options: ["MOLAP, EOLAP y COLAP", "MOLAP, ZOLAP y RTOLAP", "MOLAP, ROLAP y RTOLAP", "EOLAP, NOLAP, ZOLAP y OLAP"],
        correctAnswer: 2,
        feedback: "Los sistemas OLAP se clasifican en MOLAP, ROLAP y RTOLAP."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En COBIT 2019, ¿cuál de las siguientes opciones representa un principio del sistema de gobierno?",
        options: [
            "Basado en un modelo conceptual",
            "Alineado con los estándares más relevantes",
            "Abierto y flexible, permitiendo la adición de nuevo contenido",
            "Adaptarse a las necesidades de la empresa"
        ],
        correctAnswer: 3,
        feedback: "COBIT 2019 establece como principio 'Adaptarse a las necesidades de la empresa'."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Qué aspecto forma parte de la 'fase de arranque, sub-fase 5.4' según el Modelo Corporativo Marco de Implantaciones (MCMI)?",
        options: [
            "Realizar copias de seguridad de los sistemas destino",
            "Configurar el sistema con posterioridad al arranque",
            "Ejecutar el plan de pruebas de aceptación del sistema",
            "Preparar las sesiones de transferencia a N3"
        ],
        correctAnswer: 0,
        feedback: "En la sub-fase 5.4 de arranque se realizan copias de seguridad de los sistemas destino."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el MCMI del SAS se recogen un conjunto de entregables. ¿Cuál de los siguientes NO ES un entregable del MCMI?",
        options: [
            "[FORM02] Encuesta de satisfacción",
            "[INTE05] Plan de pruebas técnicas de las integraciones",
            "[GEST04] Registro de riesgos",
            "[MIGR01] Catálogo de reglas de negocio"
        ],
        correctAnswer: 3,
        feedback: "[MIGR01] Catálogo de reglas de negocio NO es un entregable del MCMI."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "El portal 'E-atención al profesional' y la App 'mGerhonte' son herramientas del SAS. ¿Cuál de las siguientes funcionalidades está disponible sólo en una de las aplicaciones y debería implementarse en ambas para garantizar una experiencia omnicanal?",
        options: [
            "Consulta y descarga de nóminas",
            "Informe CRP, con el detalle del abono del Complemento al Rendimiento Profesional",
            "Curriculum Digital, espacio para completar la información del profesional",
            "Modificación de cuenta bancaria"
        ],
        correctAnswer: 2,
        feedback: "El Curriculum Digital está disponible sólo en una de las aplicaciones y debería implementarse en ambas para garantizar una experiencia omnicanal."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el artículo 39 de la Ley 9/2017, es causa de nulidad de derecho administrativo:",
        options: [
            "La insuficiencia de crédito en el período vigente",
            "La falta de publicación del anuncio de licitación en el medio de publicidad en que sea preceptivo",
            "El incumplimiento leve de normas de derecho de la Unión Europea en materia de contratación pública",
            "La observancia por parte del órgano de contratación del plazo para la formalización del contrato"
        ],
        correctAnswer: 1,
        feedback: "La falta de publicación del anuncio de licitación en el medio de publicidad preceptivo es causa de nulidad de derecho administrativo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la Ley Orgánica 3/2018, en relación con el derecho de acceso, ¿en qué casos un responsable del tratamiento podrá limitar el ejercicio del derecho de acceso?",
        options: [
            "Cuando el interesado no haya justificado suficientemente su solicitud",
            "Cuando el acceso implique un esfuerzo desproporcionado para el responsable",
            "Cuando el acceso pueda afectar negativamente a los derechos y libertades de terceros",
            "Cuando el interesado haya ejercido previamente el derecho de acceso en el mismo año"
        ],
        correctAnswer: 2,
        feedback: "El responsable puede limitar el acceso cuando pueda afectar negativamente a los derechos y libertades de terceros."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el Real Decreto 203/2021, ¿cuál de los siguientes es un principio que el sector público deberá respetar según su Artículo 2?",
        options: ["Flexibilidad", "Accesibilidad", "Portabilidad", "Disponibilidad"],
        correctAnswer: 1,
        feedback: "El artículo 2 del RD 203/2021 establece la accesibilidad como uno de los principios que debe respetar el sector público."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la Ley 9/2017, en relación con la prórroga de los contratos públicos:",
        options: [
            "La prórroga debe ser aprobada por la mesa de contratación y ratificada por el empresario",
            "La prórroga será obligatoria para el empresario, siempre que su preaviso se produzca al menos con 1 mes de antelación",
            "En determinadas circunstancias, cuando al vencimiento de un contrato no se hubiera formalizado el nuevo, se podrá prorrogar el originario un máximo de 12 meses",
            "Pueden preverse una o varias prórrogas siempre que las características del contrato permanezcan inalterables"
        ],
        correctAnswer: 3,
        feedback: "La Ley 9/2017 permite prever una o varias prórrogas siempre que las características del contrato permanezcan inalterables."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "El sistema LetSAS está diseñado para que se pueda administrar con diferentes herramientas. ¿Cuál de las siguientes combinaciones es correcta para desempeñar dichas tareas de administración?",
        options: [
            "Panel Web local, telnet, ssh al puerto 443 y Altiris",
            "Terminal de Consola, Panel Web local y Altiris",
            "Telnet al puerto 443, Panel Web local y Altiris",
            "AyudaDigital y Altiris"
        ],
        correctAnswer: 1,
        feedback: "La combinación correcta para administrar LetSAS es Terminal de Consola, Panel Web local y Altiris."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el centro de servicios al usuario del SAS, la aplicación 'ayudaDIGITAL'. En relación a la versión de escritorio, ¿cuál de las siguientes afirmaciones NO es correcta?",
        options: [
            "Te avisará cuando se aproxime la fecha de renovación de la contraseña de tu usuario DMSAS",
            "Te ayuda a convertir teléfonos de la Red Corporativa",
            "Te enviará notificaciones con información de interés",
            "Te permitirá cambiar la contraseña de DMSAS cuando esté a punto de caducar"
        ],
        correctAnswer: 1,
        feedback: "La versión de escritorio de ayudaDIGITAL NO ayuda a convertir teléfonos de la Red Corporativa."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Con cuál de los siguientes protocolos opera el comando de Windows 'tracert -d 10.20.30.40'?",
        options: ["TTLvD", "DNS", "UDP", "ICMP"],
        correctAnswer: 3,
        feedback: "El comando tracert opera con el protocolo ICMP."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de las siguientes afirmaciones describe mejor el propósito de una auditoría informática?",
        options: [
            "Evaluar la eficiencia de los procesos de negocio de una organización",
            "Revisar y mejorar la experiencia del usuario en aplicaciones de software",
            "Examinar y evaluar la seguridad, integridad y eficiencia de los sistemas de información de una organización",
            "Aconsejar el desarrollo de nuevas aplicaciones de software para mejorar la productividad"
        ],
        correctAnswer: 2,
        feedback: "La auditoría informática examina y evalúa la seguridad, integridad y eficiencia de los sistemas de información."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el ámbito del diseño de un centro de proceso de datos, ¿cuál de las siguientes definiciones correspondería al concepto de RTO?",
        options: [
            "Cálculo de cuánta pérdida de datos puede experimentar una compañía",
            "Tiempo que una aplicación puede estar inactiva sin causar un daño significativo y tiempo dedicado a restaurarla",
            "Resistencia al fuego que deben tener las puertas y muros",
            "Normativa en cuanto a la retención de oxígeno que deben cumplir los gases propelentes"
        ],
        correctAnswer: 1,
        feedback: "RTO (Recovery Time Objective) es el tiempo que una aplicación puede estar inactiva y el tiempo dedicado a restaurarla."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación al modelo OSI, ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "La capa 8, o de usuario, define las funciones que el sistema de comunicación le ofrece al usuario",
            "La arquitectura TCP/IP no se adapta exactamente al modelo OSI, mezclando algunas de las capas de OSI en su definición",
            "La capa 1, o física, garantiza la comunicación extremo a extremo",
            "Todas las capas del modelo OSI disponen de funciones de códigos de redundancia"
        ],
        correctAnswer: 1,
        feedback: "TCP/IP no se adapta exactamente al modelo OSI, mezclando algunas de las capas de OSI en su definición."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación al modelo OSI, ¿cuál de las siguientes afirmaciones correspondería a las funciones de la capa número 3 o 'capa de red'?",
        options: [
            "Gestiona el direccionamiento lógico y el enrutamiento de datos entre diferentes redes",
            "Garantiza la transferencia de datos libre de errores entre dos dispositivos directamente conectados",
            "Se encarga de la traducción de los datos para asegurar su correcta interpretación",
            "Se encarga de la transmisión a través del medio físico"
        ],
        correctAnswer: 0,
        feedback: "La capa de red (Capa 3) gestiona el direccionamiento lógico y el enrutamiento de datos entre diferentes redes."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación a la arquitectura TCP/IP y el servicio DNS, el puerto definido para la transferencia de zonas DNS es:",
        options: ["El puerto 53", "El puerto 54", "El puerto 443", "El puerto 1521"],
        correctAnswer: 0,
        feedback: "El puerto estándar para la transferencia de zonas DNS es el puerto 53."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según el ámbito de la auditoría informática, ¿qué auditoría evalúa la seguridad de la infraestructura tecnológica?",
        options: ["Auditoría de desarrollo de software", "Auditoría de cumplimiento", "Auditoría de procesos", "Auditoría de seguridad"],
        correctAnswer: 3,
        feedback: "La auditoría de seguridad evalúa la seguridad de la infraestructura tecnológica."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál es la característica principal de un ataque DDoS?",
        options: [
            "El atacante utiliza vulnerabilidades del sistema final para acceder de forma no detectada",
            "Es como un ataque DoS, pero coordinando infinidad de equipos para que realicen el ataque a la vez",
            "El atacante utiliza señuelos para engañar al usuario e infectar el sistema atacado",
            "Rapta el ancho de banda del sistema atacado para descifrar el tráfico"
        ],
        correctAnswer: 1,
        feedback: "Un ataque DDoS coordina múltiples equipos para realizar un ataque DoS a la vez."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación a un sistema operativo Linux y con respecto a la gestión de usuarios y el concepto UID, ¿cuál de estas afirmaciones es correcta?",
        options: [
            "Dos usuarios de Linux nunca pueden tener el mismo UID",
            "El UID identifica a qué grupo de ACLs pertenece un login concreto",
            "El UID es el código interno que usa el sistema operativo para saber a quién pertenecen determinados objetos",
            "El comando chown fija el UID de un proceso en memoria"
        ],
        correctAnswer: 2,
        feedback: "El UID es el código interno que usa el sistema operativo para saber a quién pertenecen determinados objetos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación con la Red Corporativa de Telecomunicaciones de la Junta de Andalucía (RCJA) y la Red SARA, ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "La Red SARA permite compartir información entre administraciones públicas y tiene conexiones específicas con la Red RCJA",
            "La Red RCJA está incluida en la licitación de la Red SARA",
            "La Red SARA es la red de emergencias usada por 061, por lo que tiene conexión directa con la Red RCJA",
            "Son dos redes de entidades públicas que no tienen ningún punto en común"
        ],
        correctAnswer: 0,
        feedback: "La Red SARA permite compartir información entre administraciones públicas y tiene conexiones específicas con la Red RCJA."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En un sistema operativo Linux, ¿cuál de las siguientes expresiones sería correcta en relación con las funciones del comando sudo?",
        options: [
            "El comando sudo fue creado originalmente en el año 1995",
            "La ejecución de un comando mediante sudo necesita introducir siempre la password del usuario que lanza el comando",
            "La ejecución de un comando mediante sudo necesita introducir siempre la password del usuario con el que se ejecutará el comando",
            "Permite a un usuario ejecutar programas como usuario root sin conocer la clave de este y sin introducir la clave del primero"
        ],
        correctAnswer: 3,
        feedback: "sudo permite a un usuario ejecutar programas como usuario root sin conocer la clave de este."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En relación con las técnicas de almacenamiento basado en objetos, ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "El almacenamiento de objetos tipo Amazon S3 permite almacenar ficheros en un cluster de servidores, la unidad mínima de consumo se conoce como 'locker'",
            "Amazon S3 es un estándar de almacenamiento de ficheros que se ha liberado para poder usarlo dentro del tipo de licencia GPL",
            "Amazon S3 es un servicio que sólo puede consumirse en entornos cloud de Amazon",
            "Amazon S3 permite almacenar elementos de forma abstracta, desvinculando la estructura interna del almacenamiento del acceso al fichero"
        ],
        correctAnswer: 3,
        feedback: "El almacenamiento S3 permite almacenar elementos de forma abstracta, desvinculando la estructura interna del acceso al fichero."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Para cuál de las siguientes aplicaciones de videoconferencia se usa de forma nativa el estándar H.323 para servicios multimedia sobre redes de paquetes?",
        options: ["Unify Circuit", "Microsoft Teams", "Zoom", "Ninguno de ellos usa H.323 de forma nativa"],
        correctAnswer: 3,
        feedback: "Ninguna de las aplicaciones mencionadas usa H.323 de forma nativa."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de las siguientes opciones corresponde con un sistema de redundancia en el almacenamiento de datos?",
        options: ["Erasure Code", "RAID 0", "Almacenamiento S3", "Hyperconverged Data Resilience"],
        correctAnswer: 0,
        feedback: "Erasure Code es un sistema de redundancia en el almacenamiento de datos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En un sistema operativo multiusuario, en referencia a un mensaje de error 'segmentation fault', ¿cuál de estas afirmaciones es correcta?",
        options: [
            "Es una advertencia de que un proceso de usuario básico ha conseguido leer información de otro proceso con privilegios",
            "Es la forma de comunicar el error de ejecución 'null pointer exception' del lenguaje C",
            "El antivirus ha detectado que un proceso de malware está accediendo a segmentos del disco duro no permitidos",
            "Es el mensaje del sistema operativo al eliminar un proceso que está intentando acceder a zonas de memoria restringidas"
        ],
        correctAnswer: 3,
        feedback: "Un 'segmentation fault' es el mensaje del sistema operativo al eliminar un proceso que intenta acceder a zonas de memoria restringidas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "¿Cuál de las siguientes respuestas corresponde con una de las aplicaciones básicas del QoS en la red de datos del SAS?",
        options: [
            "Garantizar la calidad de los servicios VoIP",
            "Garantizar la redundancia de los enlaces WAN de RCJA v5",
            "Cualificar los paquetes para prevenir ataques IDS",
            "Cualificar los paquetes de servicio cuando se usa H.232 para videoconferencias"
        ],
        correctAnswer: 0,
        feedback: "QoS se aplica para garantizar la calidad de los servicios VoIP."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "En el ámbito de los servicios de datos corporativos y en relación con las siglas ZTNA, ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "ZTNA define un formato de cifrado WAN dentro de la Red Corporativa de la Junta de Andalucía",
            "ZTNA es el acrónimo para referenciar las Redes de Área Extensa Definidas por Software",
            "ZTNA permite un acceso seguro a las aplicaciones y recursos de una empresa mejorando la seguridad de las conexiones VPN",
            "ZTNA no está reconocido por el CCN como una forma de acceso segura"
        ],
        correctAnswer: 2,
        feedback: "ZTNA (Zero Trust Network Access) permite un acceso seguro a las aplicaciones y recursos mejorando la seguridad de las conexiones VPN."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019",
        question: "Según la normativa de la Junta de Andalucía y el código de colores usado en los enlaces de RJ45 para el conexionado de cable UTP, ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "Define un único orden posible de disposición de los hilos",
            "Define dos órdenes posibles de disposición de los hilos",
            "Define tres órdenes posibles de disposición de los hilos",
            "No define un orden de disposición de los hilos"
        ],
        correctAnswer: 1,
        feedback: "La normativa define dos órdenes posibles de disposición de los hilos (T568A y T568B)."
    },

    // ============================================================
    // ==================== CASOS PRÁCTICOS =======================
    // ============================================================

    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 1: ¿Cuál es el objetivo principal de implantar un Data Warehouse en el SAS?",
        options: [
            "Limitar el acceso a los datos a un sólo analista",
            "Almacenar temporalmente los documentos en papel del archivo clínico",
            "Sustituir las aplicaciones transaccionales por un esquema puramente analítico",
            "Integrar datos de distintos sistemas operacionales para proporcionar información histórica y analítica"
        ],
        correctAnswer: 3,
        feedback: "El objetivo principal de un Data Warehouse es integrar datos de distintos sistemas operacionales para proporcionar información histórica y analítica."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 1: ¿Qué ventaja ofrece la arquitectura OLAP en este contexto?",
        options: [
            "Realizar análisis multidimensional sobre grandes volúmenes de datos de forma ágil",
            "Garantizar la integridad referencial en los sistemas transaccionales",
            "Imposibilitar la representación gráfica de los informes",
            "Las mismas ventajas que la arquitectura PALO"
        ],
        correctAnswer: 0,
        feedback: "OLAP permite realizar análisis multidimensional sobre grandes volúmenes de datos de forma ágil."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 1: En el caso descrito, los cuadros de mando permitirán:",
        options: [
            "Reemplazar por completo las bases de datos transaccionales",
            "Sustituir completamente los sistemas de información clínicos por gráficos interactivos",
            "Mostrar gráficos y KPIs relevantes en tiempo real, facilitando la interpretación para la dirección",
            "Aumentar la complejidad de lectura al no jerarquizar la información"
        ],
        correctAnswer: 2,
        feedback: "Los cuadros de mando muestran gráficos y KPIs relevantes en tiempo real, facilitando la interpretación para la dirección."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 1: Para mantener la calidad de datos en el Data Warehouse, ¿qué proceso es habitual?",
        options: [
            "Usar plantillas ofimáticas sin control de versiones",
            "Evitar la depuración de datos para reflejar la realidad pura",
            "Descartar toda información que provenga de sistemas externos",
            "Aplicar procesos de ETL con limpieza y normalización de la información"
        ],
        correctAnswer: 3,
        feedback: "Para mantener la calidad de datos se aplican procesos de ETL con limpieza y normalización de la información."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 1: En el Data Warehouse, ¿cómo se asegura la confidencialidad en el tratamiento de datos sanitarios para BI?",
        options: [
            "Subiendo la base de datos a un servidor público para facilitar la transparencia",
            "Anonimizando los datos y regulando estrictamente los accesos y perfiles de usuario",
            "Omitiendo todo control documental y dejando la responsabilidad a la persona consultora",
            "Empleando un sistema abierto sin contraseñas para fomentar el intercambio"
        ],
        correctAnswer: 1,
        feedback: "La confidencialidad se asegura anonimizando los datos y regulando estrictamente los accesos y perfiles de usuario."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 2: ¿Qué ventaja técnica aportaría un modelo de puesto de trabajo basado en DaaS para el entorno clínico?",
        options: [
            "Mejora de la experiencia del usuario al permitir la ejecución local de aplicaciones pesadas",
            "Mayor control, mantenimiento centralizado y escalabilidad del entorno de trabajo sin comprometer la seguridad",
            "Sustitución de la necesidad de almacenamiento remoto con un modelo de sincronización punto a punto",
            "Eliminación de la dependencia de cualquier infraestructura de red o conectividad"
        ],
        correctAnswer: 1,
        feedback: "DaaS ofrece mayor control, mantenimiento centralizado y escalabilidad del entorno de trabajo sin comprometer la seguridad."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 2: En el contexto del Centro de Servicios al Usuario del SAS (ayudaDIGITAL), ¿qué función cumpliría una matriz de priorización de actividad?",
        options: [
            "Planificar la carga laboral de los técnicos de soporte",
            "Clasificar las solicitudes en función de criterios técnicos internos",
            "Determinar el nivel de criticidad y urgencia de la atención a incidencias según el impacto y tipo de servicio afectado",
            "Facilitar la automatización de tareas repetitivas"
        ],
        correctAnswer: 2,
        feedback: "Una matriz de priorización determina el nivel de criticidad y urgencia de la atención a incidencias según el impacto y tipo de servicio afectado."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 2: En el diseño de aplicaciones clínicas web en el SAS, ¿qué combinación de criterios es imprescindible para cumplir con los requisitos de accesibilidad?",
        options: [
            "Aplicación de principios de diseño visual consistente, navegación por teclado y validación automática de formularios",
            "Implementación de interfaces compatibles con lectores de pantalla, contraste cromático adecuado y cumplimiento de las WCAG 2.1 en nivel A",
            "Garantizar adaptabilidad al dispositivo, soporte para tecnologías asistivas y conformidad con WCAG 2.1 en nivel AA",
            "Desarrollo en HTML5 semántico, uso de etiquetas ARIA y cumplimiento parcial del nivel AAA"
        ],
        correctAnswer: 2,
        feedback: "Es imprescindible garantizar adaptabilidad al dispositivo, soporte para tecnologías asistivas y conformidad con WCAG 2.1 en nivel AA."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 2: En una arquitectura de escritorios virtuales implantada en el SAS, ¿cuál de las siguientes configuraciones es más adecuada para cumplir los principios del ENS?",
        options: [
            "Implementar inicio de sesión mediante credenciales personales y aplicar registros de auditoría centralizados por usuario",
            "Establecer políticas de grupo con perfiles de sesión (roles) y activar el bloqueo automático de pantalla tras inactividad",
            "Permitir acceso mediante usuarios genéricos en terminales clínicos, siempre que se use red segura interna",
            "Utilizar escritorios persistentes, con registro automático de logs de sistema operativo"
        ],
        correctAnswer: 0,
        feedback: "La configuración más adecuada es implementar inicio de sesión mediante credenciales personales y aplicar registros de auditoría centralizados por usuario."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 2: En el contexto de la evolución del puesto de trabajo en atención primaria, ¿qué característica técnica es fundamental para garantizar una experiencia multicanal eficaz y segura?",
        options: [
            "Implementación de aplicaciones web progresivas (PWA) con autenticación segura y sincronización local",
            "Desarrollo de clientes pesados compatibles con múltiples plataformas mediante instalación modular",
            "Uso de aplicaciones 'responsive' con autenticación federada y adaptación automática al dispositivo mediante detección contextual",
            "Centralización de accesos mediante escritorio remoto con segmentación por canal o tipo de usuario"
        ],
        correctAnswer: 2,
        feedback: "Es fundamental usar aplicaciones 'responsive' con autenticación federada y adaptación automática al dispositivo mediante detección contextual."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 3: Según la Orden de 2 de junio de 2017, ¿cuál será el cableado de fibra a utilizar para conectar los servidores del CPD?",
        options: [
            "Fibra multimodo OM2",
            "Fibra monomodo OM2",
            "Fibra multimodo OT3",
            "La orden citada no hace ninguna referencia concreta a qué tipo de fibra utilizar"
        ],
        correctAnswer: 3,
        feedback: "La orden citada no hace ninguna referencia concreta a qué tipo de fibra utilizar."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 3: Para optimizar el uso de las videoconferencias, en el CPD se aplicarán técnicas de QoS. ¿En qué elementos se aplicarán estas técnicas de forma más óptima?",
        options: ["Servidores WebRTC", "Cortafuegos", "Proxies", "Servidores Radius"],
        correctAnswer: 1,
        feedback: "Las técnicas de QoS se aplican de forma más óptima en los cortafuegos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 3: Se va a instalar un cortafuegos con inspección SSL ('Full SSL inspection'). ¿Cuál de las siguientes afirmaciones es correcta?",
        options: [
            "La inspección SSL puede generar multitud de errores en los clientes si no se configura adecuadamente",
            "La inspección SSL es una técnica usada en los cortafuegos de baja gama, no en los de última generación",
            "La inspección SSL es una técnica utilizada principalmente en los servidores web, no en los cortafuegos",
            "La inspección SSL no se puede realizar porque rompería la confianza cliente-servidor del tráfico HTTPS"
        ],
        correctAnswer: 0,
        feedback: "La inspección SSL puede generar multitud de errores en los clientes si no se configura adecuadamente."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 3: La Agencia Digital de Andalucía quiere implantar un sistema de gestión de la seguridad de la información. ¿Cuál de las normativas siguientes sería la adecuada?",
        options: ["ISO 9001", "ISO 14001", "ISO 20000-1", "ISO 27001"],
        correctAnswer: 3,
        feedback: "La ISO 27001 es la normativa adecuada para un sistema de gestión de la seguridad de la información."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 3: De cara a implantar técnicas de virtualización de servidores, ¿cuál de los siguientes es un hipervisor que permita este tipo de técnicas?",
        options: ["AHV", "ESI", "HPV", "VMW"],
        correctAnswer: 0,
        feedback: "AHV (Acropolis Hypervisor) es un hipervisor que permite técnicas de virtualización de servidores."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 4: En relación con la Estrategia de Salud Digital en Andalucía, ¿cuál de las siguientes afirmaciones refleja una característica clave del modelo de gobernanza TIC en el SAS?",
        options: [
            "El modelo de gobernanza TIC delega la planificación tecnológica en los equipos TIC provinciales",
            "El modelo promueve una estructura centralizada sin participación de los centros asistenciales",
            "El modelo integra la planificación TIC con la planificación estratégica asistencial y directiva",
            "El modelo no se ocupa de la evaluación periódica de resultados TIC en los centros"
        ],
        correctAnswer: 2,
        feedback: "El modelo de gobernanza TIC integra la planificación TIC con la planificación estratégica asistencial y directiva."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 4: En la planificación del nuevo HIS, ¿qué documento debe recoger los objetivos estratégicos y tecnológicos a medio-largo plazo?",
        options: ["El plan de estrategia digital", "El plan anual TIC", "El plan de sistemas de información", "El plan de seguridad de la información"],
        correctAnswer: 2,
        feedback: "El plan de sistemas de información recoge los objetivos estratégicos y tecnológicos a medio-largo plazo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 4: Para la adquisición de una nueva plataforma de interoperabilidad clínica, ¿cuál de los siguientes elementos debe incluirse obligatoriamente en el pliego de prescripciones técnicas?",
        options: [
            "Una referencia genérica al producto comercial más implantado en el mercado",
            "La exigencia de uso exclusivo de software propietario",
            "Requisitos funcionales detallados y criterios de interoperabilidad conforme a estándares reconocidos",
            "La obligación de contratar únicamente con empresas tecnológicas radicadas en Andalucía"
        ],
        correctAnswer: 2,
        feedback: "El pliego debe incluir requisitos funcionales detallados y criterios de interoperabilidad conforme a estándares reconocidos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 4: ¿Cuál de los siguientes aspectos facilita la implantación efectiva de un modelo de gestión del cambio en organizaciones sanitarias?",
        options: [
            "Centrarse principalmente en la capacitación técnica del personal clínico",
            "Evitar la participación de los usuarios finales en la fase de diseño",
            "Establecer canales de comunicación bidireccional con los grupos de interés",
            "Mantener los procesos tradicionales del centro"
        ],
        correctAnswer: 2,
        feedback: "Establecer canales de comunicación bidireccional con los grupos de interés facilita la implantación efectiva de un modelo de gestión del cambio."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 4: ¿Qué modelo de mejora continua se recomienda para la evaluación y optimización de los servicios TIC de un hospital público andaluz?",
        options: [
            "Modelo de planificación financiera CAPEX-OPEX",
            "Modelo EFQM de excelencia con enfoque en procesos",
            "Modelo de calidad total Kaizen",
            "Modelo Lean de optimización de modelos de gestión"
        ],
        correctAnswer: 1,
        feedback: "El modelo EFQM de excelencia con enfoque en procesos se recomienda para la evaluación y optimización de los servicios TIC."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 5: ¿Cuál es la utilidad de realizar un Estudio de Viabilidad del Proyecto (EVS) antes de su ejecución?",
        options: [
            "Garantizar la alineación con PMBOK",
            "Obtener una primera estimación del alcance, coste y tiempo del proyecto",
            "Reducir la incertidumbre inicial y evaluar la capacidad real de la organización para llevar a cabo el proyecto",
            "Todas las opciones son correctas"
        ],
        correctAnswer: 3,
        feedback: "Todas las opciones son correctas: el EVS garantiza alineación, obtiene estimaciones y reduce incertidumbre."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 5: Para el enfoque del desarrollo se va a utilizar Domain-Driven Design (DDD). ¿Cuál es su principal ventaja?",
        options: [
            "Mejora la claridad y la alineación entre el software y las necesidades de negocio",
            "Reduce la necesidad de documentación en el proceso de desarrollo",
            "Evita la necesidad de desarrollar múltiples servicios para cada modelo",
            "Evita cambios en los requisitos de software"
        ],
        correctAnswer: 0,
        feedback: "Domain-Driven Design mejora la claridad y la alineación entre el software y las necesidades de negocio."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 5: Para su interfaz de usuario trabajaremos con micro-frontends. ¿En cuántos componentes se divide la arquitectura de un micro-frontend según la normativa TIC del SAS?",
        options: ["1", "2", "3", "4"],
        correctAnswer: 2,
        feedback: "La arquitectura de un micro-frontend se divide en 3 componentes según la normativa TIC del SAS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 5: Para adaptarse a la normativa TIC del SAS para los informes, ¿qué se debería tener en cuenta?",
        options: [
            "No se recomienda el uso de Javascript",
            "El formato recomendado para publicar informes es HTML",
            "El contenido debe estar codificado en UTF-16",
            "Se pueden utilizar comillas simples"
        ],
        correctAnswer: 1,
        feedback: "El formato recomendado para publicar informes es HTML según la normativa TIC del SAS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 5: Para su puesta en producción se va a escoger un grupo de usuarios para beta-testing. ¿Qué patrón de despliegue podría utilizarse?",
        options: ["Honeypot Pattern", "Prototype Pattern", "Canary Development", "Observer Pattern"],
        correctAnswer: 2,
        feedback: "Canary Development es el patrón de despliegue que utiliza un grupo de usuarios para beta-testing antes del despliegue completo."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 6: En el proyecto, se decide emplear un desarrollo orientado a pruebas (TDD). ¿En qué consiste su principal dinámica?",
        options: [
            "Escribir primero el código funcional y luego redactar las pruebas una vez finalizado",
            "Se define la prueba, se escribe el código para superarla y se refactoriza",
            "Eliminar todas las pruebas unitarias en favor de pruebas manuales",
            "Basar todo en un backlog estático sin refactorización"
        ],
        correctAnswer: 1,
        feedback: "TDD consiste en definir la prueba, escribir el código para superarla y refactorizar."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 6: ¿Por qué se debería fomentar la reutilización de componentes en el desarrollo?",
        options: [
            "Facilita la reducción de tiempos y costes",
            "Limita la evolución independiente de cada subsistema",
            "No se podría usar porque es incompatible con cualquier metodología ágil",
            "Aumenta el coste y la complejidad del proyecto"
        ],
        correctAnswer: 0,
        feedback: "La reutilización de componentes facilita la reducción de tiempos y costes."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 6: Para controlar la configuración del software y gestionar los cambios, se adopta Git. ¿Cuál es una buena práctica en la rama de producción?",
        options: [
            "Mover todo el historial a la rama de producción sin revisiones",
            "Forzar commits directos de todos los desarrolladores sin revisión",
            "Mantener la rama estable, integrando cambios a través de otras ramas",
            "Eliminar el etiquetado de versiones para simplificar el proceso de los merges"
        ],
        correctAnswer: 2,
        feedback: "Una buena práctica es mantener la rama estable, integrando cambios a través de otras ramas."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 6: Se desea incluir pruebas de rendimiento en la integración continua. ¿Cuál sería la herramienta o metodología recomendada?",
        options: [
            "jMeter o herramientas equivalentes para simular cargas y transacciones, generando reportes automatizados",
            "Realizar pruebas manuales sin definir escenarios de concurrencia",
            "Contratar una empresa externa que pruebe al final del proyecto",
            "Obviar el rendimiento en entornos de preproducción"
        ],
        correctAnswer: 0,
        feedback: "jMeter o herramientas equivalentes son recomendadas para simular cargas y transacciones, generando reportes automatizados."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 6: En la fase de pruebas, se consultan las directrices de la ISO 29119. Esto significa:",
        options: [
            "Someter la documentación y el proceso de pruebas a un estándar reconocido para asegurar su consistencia y trazabilidad",
            "Reducir las pruebas de multiplexación al mínimo legal exigido",
            "Adaptar únicamente la parte de documentación sobre interfaces gráficas",
            "Limitar las revisiones a informes genéricos sin casos de test definidos"
        ],
        correctAnswer: 0,
        feedback: "Consultar ISO 29119 significa someter la documentación y el proceso de pruebas a un estándar reconocido."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 7: ¿Cuál de los siguientes protocolos sirve para crear redes privadas virtuales?",
        options: ["IPTP", "L2IP", "L2TP", "PPDP"],
        correctAnswer: 2,
        feedback: "L2TP (Layer 2 Tunneling Protocol) sirve para crear redes privadas virtuales."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 7: Para que los usuarios remotos puedan resolver nombres internos, la VPN debe enviar peticiones DNS a un servidor concreto. ¿Cuál de los siguientes sería correcto?",
        options: [
            "10.234.22.1",
            "dns.sas.juntadeandalucia.es",
            "8.8.8.8",
            "Todos los anteriores resolverían correctamente el nombre del recurso interno"
        ],
        correctAnswer: 1,
        feedback: "El servidor DNS interno dns.sas.juntadeandalucia.es es el correcto para resolver nombres de recursos internos."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 7: Para implementar una estrategia de SD-WAN, ¿cuál de las siguientes definiciones correspondería con este concepto?",
        options: [
            "Una técnica implementada en los cortafuegos corporativos que permite hacer escaneos dinámicos de los dispositivos destino",
            "Una categoría de tecnologías que permiten gestionar una red y ajustar la topología de la misma mediante software",
            "Una revisión de la arquitectura del modelo OSI que ahora tiene en cuenta origen y destino de las comunicaciones remotas",
            "Un nuevo tipo de líneas de comunicación de fibra óptica que permiten conexiones a grandes distancias"
        ],
        correctAnswer: 1,
        feedback: "SD-WAN es una categoría de tecnologías que permiten gestionar una red y ajustar la topología mediante software."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 7: En relación con las técnicas criptográficas para cifrado asimétrico, ¿cuál de estas referencias correspondería con un algoritmo para este tipo de cifrado?",
        options: ["SHA-256", "RSA", "RS-422", "F5"],
        correctAnswer: 1,
        feedback: "RSA es un algoritmo de cifrado asimétrico."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 7: Si para mejorar el servicio prestado con las VPNs quisiéramos usar el paradigma DaaS, ¿con cuál de estas soluciones software se podría implementar?",
        options: ["Hyper-V HPC", "Citrix VDA", "Nutanix Karbon", "Oracle RAS"],
        correctAnswer: 1,
        feedback: "Citrix VDA (Virtual Delivery Agent) es una solución para implementar DaaS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 8: En el contexto de la financiación de proyectos TIC, ¿cuál de las siguientes NO corresponde a una fuente de financiación actualmente en ejecución en el SAS?",
        options: ["Next Generation EU", "PERTE", "SIVAIN", "E2HEALTH EU"],
        correctAnswer: 2,
        feedback: "SIVAIN no corresponde a una fuente de financiación actualmente en ejecución en el SAS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 8: Según la Ley Orgánica 3/2018, los datos anonimizados:",
        options: [
            "Siguen considerándose datos personales",
            "Están fuera del ámbito de aplicación de esta ley",
            "Deben ser eliminados en un plazo de 5 años",
            "Sólo pueden usarse con consentimiento expreso del usuario"
        ],
        correctAnswer: 1,
        feedback: "Según la Ley Orgánica 3/2018, los datos anonimizados están fuera del ámbito de aplicación de esta ley."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 8: En este tipo de iniciativas es importante evitar el vendor-locking. ¿En qué consiste ese concepto?",
        options: [
            "En la posibilidad de utilizar múltiples plataformas sin restricciones",
            "En la implementación de estándares abiertos en la comunicación",
            "En la dependencia de un proveedor específico que dificulta la migración a otra plataforma",
            "En la disposición de la solución en omnicanalidad"
        ],
        correctAnswer: 2,
        feedback: "Vendor-locking consiste en la dependencia de un proveedor específico que dificulta la migración a otra plataforma."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 8: Para integrar el sistema de videollamada se ha decidido seguir el modelo de ciclo de vida de software en espiral. ¿Cuáles son las dimensiones clave de este modelo?",
        options: ["Radial y angular", "Angular y secuencial", "Secuencial y lineal", "Lineal y radial"],
        correctAnswer: 0,
        feedback: "El modelo en espiral se basa en las dimensiones radial (coste) y angular (progreso)."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 8: Para disponer de toda la información se apuesta por utilizar un Data Warehouse. ¿Cuál es la principal característica de una tabla de hechos en un Data Warehouse?",
        options: [
            "Contiene datos descriptivos y categorías",
            "Almacena métricas y valores numéricos para análisis",
            "Sólo dispone de claves primarias",
            "Se utiliza para almacenar información temporal"
        ],
        correctAnswer: 1,
        feedback: "La principal característica de una tabla de hechos es que almacena métricas y valores numéricos para análisis."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 9: ¿Cuál de estas herramientas es la ideal para gestionar y monitorizar dispositivos móviles y aplicar políticas de seguridad de forma centralizada?",
        options: ["CMS", "CrowdStrike", "GPOs de Active Directory", "MDM"],
        correctAnswer: 3,
        feedback: "MDM (Mobile Device Management) es la herramienta ideal para gestionar y monitorizar dispositivos móviles."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 9: Desde el punto de vista de la administración y gestión del sistema operativo Android, ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "Los dispositivos Android no pueden participar como integrantes de un DDoS porque no pueden tener virus",
            "El sistema operativo Android no hace separación de memoria por usuario, ya que no es un entorno multiusuario",
            "Los dispositivos Android pueden sufrir ataques de virus de tipo troyano",
            "El sistema operativo Android evita que estos dispositivos puedan sufrir ataques de virus de tipo troyano"
        ],
        correctAnswer: 2,
        feedback: "Los dispositivos Android pueden sufrir ataques de virus de tipo troyano."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 9: Se desea usar tecnologías MIMO para mejorar la cobertura wifi. ¿Cuál de las cuatro definiciones siguientes corresponde a esta tecnología?",
        options: [
            "Machine Intelligence Management Operations",
            "Media Interactive Media Optimization",
            "Modular Integrated Micro-Optics",
            "Multiple-input Multiple-output"
        ],
        correctAnswer: 3,
        feedback: "MIMO significa Multiple-input Multiple-output."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 9: Cuando un dispositivo móvil tenga un problema de conexión a red corporativa, ¿dónde debe registrar la incidencia el profesional asistencial?",
        options: [
            "En el sistema de gestión de garantías del fabricante",
            "En SioWeb por ser un problema con un móvil corporativo",
            "En NAOS por ser un problema de conexión de Red Corporativa",
            "En AyudaDigital por ser un problema en un sistema del SAS"
        ],
        correctAnswer: 3,
        feedback: "Según la política de soporte, la incidencia se registrará en AyudaDigital por ser un problema en un sistema del SAS."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 9: ¿Con cuál de estas tecnologías se deberían cifrar los discos de portátiles con Windows para evitar fugas de datos en caso de pérdida o robo?",
        options: ["BitDefender", "BitJail", "BitLocker", "BitSrike"],
        correctAnswer: 2,
        feedback: "BitLocker es la tecnología de cifrado de discos de Windows."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 10: Según la Ley 9/2017, la adquisición de un programa de ordenador desarrollado a medida es un:",
        options: ["Contrato de servicios", "Contrato de suministros", "Contrato de concesión de servicios", "Contrato de obra"],
        correctAnswer: 0,
        feedback: "La adquisición de un programa de ordenador desarrollado a medida es un contrato de servicios."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 10: La certificación ISO/IEC 20000, ¿cada cuánto tiempo tiene que renovarse?",
        options: ["Cada 3 años", "Cada 5 años", "Cada año", "No es necesario renovarla"],
        correctAnswer: 0,
        feedback: "La certificación ISO/IEC 20000 debe renovarse cada 3 años."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 10: Para organizar el proyecto se propone definir una matriz RASCI. ¿Cuál de las siguientes afirmaciones es INCORRECTA?",
        options: [
            "Un rol puede desempeñar más de una función dentro de una misma actividad",
            "No se puede tener más de un responsable ('R') asignado a una tarea",
            "Si no se implementa correctamente, la Matriz RASCI puede volverse burocrática",
            "La Matriz RASCI contribuye a evitar la duplicación de esfuerzos dentro de un proyecto"
        ],
        correctAnswer: 1,
        feedback: "Es INCORRECTO que no se pueda tener más de un responsable asignado a una tarea; en RASCI solo debe haber un 'Responsable' por tarea."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 10: Según la Norma Técnica de Interoperabilidad de Documento Electrónico, ¿cuál NO ES un metadato obligatorio?",
        options: ["Identificador", "Origen", "Nombre de formato", "Fecha de modificación"],
        correctAnswer: 1,
        feedback: "El metadato 'Origen' NO es un metadato obligatorio según la Norma Técnica de Interoperabilidad de Documento Electrónico."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "CASO PRÁCTICO 10: Según el MCMI del SAS, ¿cuál de las siguientes fases se centra en la ampliación del alcance de la implantación?",
        options: ["Fase 5: arranque", "Fase 6: consolidación", "Fase 7: extensión", "Fase 8: paso a N3"],
        correctAnswer: 2,
        feedback: "La Fase 7: extensión se centra en la ampliación del alcance de la implantación."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "Una de las normas o estándares para la interoperabilidad entre distintos gestores de bases de datos relacionales es:",
        options: [
            "El uso de interfaces como ODBC y JDBC, que permiten conectar aplicaciones con distintos motores de bases de datos de forma uniforme",
            "La adopción de un único sistema operativo para todos los servidores",
            "El uso de cifrado privativo que refuerza la seguridad",
            "La restricción a un único tipo de dato por tabla"
        ],
        correctAnswer: 0,
        feedback: "ODBC y JDBC son interfaces que permiten conectar aplicaciones con distintos motores de bases de datos de forma uniforme."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "Dentro del ámbito de aplicación del Esquema Nacional de Seguridad (RD 311/2022), ¿cuál de las siguientes afirmaciones es correcta?",
        options: [
            "No se aplica a los sistemas de información de las entidades del sector privado",
            "No es necesario aplicarla a los sistemas del sector privado cuando cuentan con política de seguridad",
            "Se aplica a los sistemas de información del sector privado cuando presten servicios o provean soluciones a las entidades del sector público",
            "Se aplica solo a los sistemas que realicen tratamiento de datos personales"
        ],
        correctAnswer: 2,
        feedback: "El ENS se aplica a los sistemas de información del sector privado cuando presten servicios o provean soluciones a las entidades del sector público."
    },
    {
        topic: "Examen Técnico FAS TI - Acceso Libre 2019 - Práctico",
        question: "¿Qué característica clave diferencia al estándar xAPI respecto a SCORM en sistemas de e-learning?",
        options: [
            "xAPI no permite el seguimiento del aprendizaje fuera del LMS",
            "xAPI permite registrar experiencias de aprendizaje distribuidas, incluso fuera del aula virtual",
            "xAPI se limita a contenidos sin interacción del alumno",
            "SCORM es compatible con apps móviles, mientras que xAPI es exclusivo de plataformas web"
        ],
        correctAnswer: 1,
        feedback: "xAPI permite registrar experiencias de aprendizaje distribuidas, incluso fuera del aula virtual, a diferencia de SCORM."
    }
];

// Si necesitas exportarlo para usarlo en Node.js o con módulos
if (typeof module !== 'undefined' && module.exports) {
    module.exports = examen1_TFA_2019;
}